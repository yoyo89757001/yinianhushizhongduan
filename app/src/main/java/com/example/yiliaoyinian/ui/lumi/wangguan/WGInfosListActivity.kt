...
class WGInfosListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_w_g_infos_list)
    ...
    fun initview(){
        ...
        rl1.setOnClickListener {
            val builder = EditTextDialogBuilder(this@WGInfosListActivity)
            builder.setTitle("修改设备名称")
                    .setSkinManager(QMUISkinManager.defaultInstance(this@WGInfosListActivity))
        ...
        rl4.setOnClickListener {
            startActivity(Intent(this@WGInfosListActivity, GengDuocActivity::class.java).putExtra("did", did))
        }
        rl5.setOnClickListener {
            startActivity(Intent(this@WGInfosListActivity, ZDHActivity1::class.java).putExtra("did", did))
        }
        rl9.setOnClickListener {
            startActivity(Intent(this@WGInfosListActivity, WangLuosActivity::class.java).putExtra("did", did))
        }
        yichu.setOnClickListener {
            val commomDialog = CommomDialog(this@WGInfosListActivity, R.style.dialogs, "确定要移除设备?") { dialog, confirm ->
                Log.d("DAFragment3", "confirm:$confirm")
                if (confirm) {
                    //退出动作
                    if (did!=null){
                        qmdialog = QMUITipDialog.Builder(this@WGInfosListActivity).setIconType(QMUITipDialog.Builder.ICON_TYPE_LOADING)?.setTipWord("正在加载")?.create()
                        qmdialog?.show()
                        loginApiunBind(did!!)
                    }
                    dialog.dismiss()
